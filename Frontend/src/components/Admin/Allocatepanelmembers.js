...
import { getGroupsById, updatepannelmemberByID } from "../services/DisplayGroupRegister";
...
const Allocatepanelmembers = () => {
  ...
  const [pannelmemberName, setpannelmemberName] = useState("");
  ...
  const handlepannelmemberName= (e) => {
    setpannelmemberName(e.target.value);
  };
  ...
  const GetData = async () => {
    ...
    setpannelmemberName(data?.data?.pannelmemberName);
  ...
  const UpdateData = async (e) => {
    ...
    if (!data?.data?.GruopLeaderEmail) {
      {   Swal.fire('error' , 'Assign pannelmember   Error ' , 'error')

      navigate("/DisplayAllGroupsAddPanelMembers");
   }
      ...
      {   Swal.fire('Congrats' , 'Assign pannelmember successfully ' , 'success')
    ...
    <div className="container-fluid px-1 px-md-5 px-lg-1 px-xl-5 py-5 mx-auto">
      <center>
        ...
        <div class="form-group">
          <label className="label" style={{color:"red" , fontSize:"30px" }}>Pannel Member Name</label>

    <select class="form-control" id="exampleFormControlSelect1"   
            value={pannelmemberName}  onChange={handlepannelmemberName} >
      <option  >Ms. Dilani</option>
      <option>Ms. Karthiga</option>
      <option>Ms. Rashini</option>
      <option>Ms. Supipi</option>
