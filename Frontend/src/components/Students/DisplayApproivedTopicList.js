...
function DisplayApproivedTopicList() {
  ...
  useEffect(async () => {
    ...
    try {
      const data = await (
        await axios.get("https://afresearchprojectmanagementtoo.herokuapp.com/topicsrequest/getAlltopiclist/")
      ).data;
...
<br/>
    ...
    <div className="row">
      ...
      <div className="col-md-9">
        ...
        <center><h1>Topic Approved  Status</h1></center>
    
        <div class="input-group">
  <div className="col-md-9">
  <h4>👌 Now You Can Search Your Group  topic status   😉</h4>
        ...
        <table className="table table-bordered " style={{marginLeft:"20%" , marginTop:"-20px" , width:""}}>
          <thead className="bs">
            <tr>
              ...
              <th  style={{color:"white" , backgroundColor:"black"}} >Gruop Leader ItNumber</th>
              ...
              .map((users) => {
                ...
                return (
                  <tr>
                    ...
                    <td>{users.GruopLeaderItNumber}</td>
                    <td>{users.Urls}</td>
                    <td>{users.Message}</td>
                    <td>{users.status}</td>
                  </tr>
...
export default DisplayApproivedTopicList;
