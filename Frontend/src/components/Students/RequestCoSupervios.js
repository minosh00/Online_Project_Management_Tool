...
import { createCoSupervisorRequest } from "../services/CosuperVisorrequest";
import Swal from 'sweetalert2';




const RequestCoSupervios = () => {
  ...
  const handleGruopLeaderEmail = (e) => {
	...
	const requestCoSupervisor = async (e) => {
		...
		const data ={
			CosupervisorID: cosuperData.CosupervisorID,
			GroupID: GroupID,
			GruopLeaderEmail: GruopLeaderEmail,
		}
		let response = await createCoSupervisorRequest(data);
		console.log("CoSuperviser reg ", response);
	...
	const getAllCosuperivsor = async () => {
		try{
			setLoading(true);
...
export default RequestCoSupervios;
